...
bool CupGeneralTCB::StartTrigger()
{
  ...
  if (nadc > 0) {
    cout << endl;
    cout << " -----------------------------------------------" << endl;
    cout << " WJ FADC NADC = " << nadc << endl;
    cout << " WJ FADC PID = ";
    for (int i = 0; i < nadc; i++) {
        auto * conf = (FADCTConf *)fConfigs->GetConfig(ADC::FADCT, i);
        if (conf->IsLinked() && conf->IsEnabled())
            fTCB->RegisterValueFADC(conf, 0);
    }
    cout << endl;

    cout << " WJ FADC DLY (registered) = ";
    for (int i = 0; i < nadc; i++) {
        auto * conf = (FADCTConf *)fConfigs->GetConfig(ADC::FADCT, i);
        if (conf->IsLinked() && conf->IsEnabled())
            fTCB->RegisterValueFADC(conf, 1);
    }
    cout << endl;

    cout << " WJ FADC THR (registered) = ";
    for (int i = 0; i < nadc; i++) {
        auto * conf = (FADCTConf *)fConfigs->GetConfig(ADC::FADCT, i);
        if (conf->IsLinked() && conf->IsEnabled())
            fTCB->RegisterValueFADC(conf, 2);
    }
    cout << endl;
    cout << " -----------------------------------------------" << endl;
    cout << endl;
  ...
  if (nadc > 0) {
    cout << endl;
    cout << " -----------------------------------------------" << endl;
    cout << " WJ SADC NADC = " << nadc << endl;
    cout << " WJ SADC PID = ";
    for (int i = 0; i < nadc; i++) {
        auto * conf = (SADCTConf *)fConfigs->GetConfig(ADC::SADCT, i);
        if (conf->IsLinked() && conf->IsEnabled())
            fTCB->RegisterValueSADC(conf, 0);
    }
    cout << endl;

    cout << " WJ SADC DLY (registered) = ";
    for (int i = 0; i < nadc; i++) {
        auto * conf = (SADCTConf *)fConfigs->GetConfig(ADC::SADCT, i);
        if (conf->IsLinked() && conf->IsEnabled())
            fTCB->RegisterValueSADC(conf, 1);
    }
    cout << endl;

    cout << " WJ SADC THR (registered) = ";
    for (int i = 0; i < nadc; i++) {
        auto * conf = (SADCTConf *)fConfigs->GetConfig(ADC::SADCT, i);
        if (conf->IsLinked() && conf->IsEnabled())
            fTCB->RegisterValueSADC(conf, 2);
    }
    cout << endl;
    cout << " -----------------------------------------------" << endl;
    cout << endl;
