    ...
    : AbsADCRaw(s, type)
{
  ...
  fTime = new unsigned long[fNCH]; // WJ: int -> long
  memset(fTime, 0, fNCH*sizeof(unsigned long)); // WJ: int -> long
    ...
    : AbsADCRaw(raw)
{
  fNCH = raw.GetNCH();
  fADC = new unsigned int[fNCH];
  fTime = new unsigned long[fNCH];
  memcpy(fADC, raw.GetADCs(), fNCH * sizeof(unsigned int));
  memcpy(fTime, raw.GetTimes(), fNCH * sizeof(unsigned long)); // WJ: int -> long
...
void SADCRawEvent::Unpack_SADC(AbsConf * config, int verbose)
{
  ...
  unsigned int itmp, peak; 
  unsigned long ltmp, peaktime; // WJ: unsigned int -> unsigned long
  for (int i = 0; i < kNCHSADC; i++) {
    ...
    peaktime = peaktime * 8;
    ltmp = fData[32 + i * 7 + 4] & 0xFF;
    peaktime = peaktime + ltmp * 1000;
    ltmp = (unsigned long)(fData[32 + i * 7 + 5] & 0xFF) << 8;
    peaktime = peaktime + ltmp * 1000;
    ltmp = (unsigned long)(fData[32 + i * 7 + 6] & 0xFF) << 16;
    peaktime = peaktime + ltmp * 1000;
    fTime[i] = peaktime;
  }
}
