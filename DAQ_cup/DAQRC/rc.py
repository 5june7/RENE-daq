...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def boot_run(self):
        ...
        if self.SplitTimeConfig.text():
            self.SplitTime = int(self.SplitTimeConfig.text())
        ...
        msg3 = 'Config file: %s\n<br>' % configfile
        msg4 = 'Split time: %d [m]<br>' % self.SplitTime
        ...
        self.RunNumber = self.RunCatalogTable.insert(shift=self.Shift,
                                                     ...
                                                     rundesc=self.RunDesc+', Split T [m] = '+str(self.SplitTime),
        ...
        split_time = (self.SplitTime * 60) # WJ: [s] -> [m]
        ...
        for daq in daqlist:
            ...
            if mode == 0:
                ...
                dopt = '-d 0 -r %d -c %s -p %d' % (run_number, config, split_time)
            elif mode == 2:
    ...
    def update_button(self):
        Fimage = 'updatecomingsoon.png' # change it as the right figure!
        Simage = 'updatecomingsoon.png' # change it as the right figure!
        image_ = 'updatecomingsoon.png'
        ...
        if os.path.isfile(Fimage):
            self.graphF.load(Fimage)
            self.FADCFIG.setPixmap(self.graphF.scaled(self.FADCFIG.width(), self.FADCFIG.height(), Qt.KeepAspectRatio))
            self.FADCFIG.setAlignment(Qt.AlignCenter)
        else:
            self.graphF.load(image_)
            self.FADCFIG.setPixmap(self.graphF.scaled(self.FADCFIG.width(), self.FADCFIG.height(), Qt.KeepAspectRatio))
            self.FADCFIG.setAlignment(Qt.AlignCenter)

        # SADC figure
        if os.path.isfile(Simage):
            self.graphS.load(Simage)
            self.SADCFIG.setPixmap(self.graphS.scaled(self.SADCFIG.width(), self.SADCFIG.height(), Qt.KeepAspectRatio))
            self.SADCFIG.setAlignment(Qt.AlignCenter)
        else: 
            self.graphS.load(image_)
            self.SADCFIG.setPixmap(self.graphS.scaled(self.SADCFIG.width(), self.SADCFIG.height(), Qt.KeepAspectRatio))
            self.SADCFIG.setAlignment(Qt.AlignCenter)
    ...
    def update_runstate(self):
        ...
        if not self.OnThisRC and self.RunState is not onlconsts.kDOWN:
            ...
            msg = '<font color="blue"><b>%s</b></font> loaded' % configfile
            self.ConfigFileLabel.setText(msg)
            self.SplitTimeConfig.setText(self.SplitTime)
            self.OnThisRC = True
        ...
        summary += '    <b>Run Number</b>: %06d/%d  ' % (
            self.RunNumber, self.SubRunNumber)
        summary += '      <b>Split Time</b>: %d [m]<br>' % self.SplitTime
    ...
    def set_runstate(self, state):
        if state == onlconsts.kDOWN:
            ...
            self.SplitTimeConfig.setEnabled(True)
        else:
            ...
            self.SplitTimeConfig.setEnabled(False)
