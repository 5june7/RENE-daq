...
void SourceData_Qsum_veto_fitting_v2_SADC_TwoPMTs(int run1, int run2, int subrun1, int subrun2){

    gStyle->SetOptStat(111111);


    char run1_str[1000000];
    sprintf(run1_str, "%06d", run1);
    char subrun1_str[1000000];
    sprintf(subrun1_str, "%05d", subrun1);

    char run2_str[1000000];
    sprintf(run2_str, "%06d", run2);
    char subrun2_str[1000000];
    sprintf(subrun2_str, "%05d", subrun2);
    ...
    double fitExpS[2] = { 50, 70 };//95;//50; // CORRECT
    double fitExpF[2] = { 120, 150 };//95;//50; // CORRECT

    double fitLanS[2] = { 120, 150 };//95;//50; // CORRECT
    ...
    //if(subrun1 < 0) chain1->Add(Form("/Data/RAW/%s/PRD/PRD_%s.*.root", run1_str, run1_str));
    if(subrun1 < 0) chain1->Add(Form("/Data/RAW/%s/PRD/PRD_%s.*_*.root", run1_str, run1_str));
    else chain1->Add(Form("/Data/RAW/%s/PRD/PRD_%s.%s.root", run1_str, run1_str, subrun1_str));
    setbranch(chain1);

    for( int iEntry = 0; iEntry < chain1->GetEntries(); iEntry++ ){
        initializing();

        if( iEntry % (chain1->GetEntries()/10) == 0 )
            cout<<iEntry<<" / "<<chain1->GetEntries()<<" processing ..."<<endl;
    ...
    //if(subrun1 < 0) chain2->Add(Form("../../Data/RAW/%s/PRD/PRD_%s.*.root", run2_str, run2_str));
    if(subrun1 < 0) chain2->Add(Form("../../Data/RAW/%s/PRD/PRD_%s.*_*.root", run2_str, run2_str));
    else chain2->Add(Form("../../Data/RAW/%s/PRD/PRD_%s.%s.root", run2_str, run2_str, subrun2_str));
    setbranch(chain2);
    ...
    for( int iEntry = 0; iEntry < chain2->GetEntries(); iEntry++ ){
        ...
        if( iEntry % (chain2->GetEntries()/10) == 0 )
            cout<<iEntry<<" / "<<chain2->GetEntries()<<" processing ..."<<endl;
    ...
    for(int ich = 0; ich < nch_fit; ich++){
        ...
        ini2[0][0] = 1; ini2[0][1] = 140; ini2[0][2] = 1; // CORRECT: MPV = ini2[ich][1]
        ini2[1][0] = 1; ini2[1][1] = 211; ini2[1][2] = 1; // CORRECT: MPV = ini2[ich][1]
